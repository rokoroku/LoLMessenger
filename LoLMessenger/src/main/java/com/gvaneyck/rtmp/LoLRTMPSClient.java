...
public class LoLRTMPSClient extends RTMPSClient
{
    ...
    public void connectAndLogin(CallbackToMessenger callback) throws IOException
    {
        connect();
        login(callback);
    }
    ...
    public void login(CallbackToMessenger callback) throws IOException
    {
        if (useGarena)
            getGarenaToken();
	...
	*/
    ...
    private void getAuthToken(CallbackToMessenger callback) throws IOException
    {
        // Initial authToken request
        String payload;
        if (useGarena)
        ...
        //{"rate":0,"reason":"account_banned","status":"FAILED","delay":10000,"banned":7647952951000}
        if (result.get("status").equals("FAILED")) {
            String message = "Error logging in: " + result.get("reason");
            callback.sendMessage(message);
        ...
        if (!result.containsKey("token"))
        {
            ...
            // Let the user know
            String message = "In login queue for " + region + ", #" + (id - cur) + " in line";
            callback.sendMessage(message);
            ...
            while (id - cur > rate)
            {
                ...
                cur = hexToInt(result.getString(nodeStr));
                message = "In login queue for " + region + ", #" + (id - cur) + " in line";
                callback.sendMessage(message);
            }
		...
		private void beatHeart(Thread thread)
		{
			while (curThread == thread)
			{
				try
				{
					...
					System.out.println("LOLRTMPSClient:HeartBeat " + id);
				...
				{
                    //System.out.println("LOLRTMPSClient:HeartBeat Exception " + e.getMessage());
                    //connected = false;
